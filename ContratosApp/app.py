...
{% if drive_url %}<section class="card"><h2>Borrador listo para revisión</h2><p>El contrato fue guardado como documento editable en Google Drive.</p><div class="notice info"><strong>Editar en Google Drive:</strong> realiza los cambios, descarga el documento como PDF y vuelve a este formulario para subirlo y enviarlo a firma.</div><a class="drive-button" href="{{ drive_url }}" target="_blank" rel="noopener">ABRIR EN GOOGLE DRIVE</a>{% if editor_configurado %}<div class="notice info"><strong>Editar en el editor de documento:</strong> guarda los cambios y presiona <strong>Volver al formulario</strong>. El PDF se descargará automáticamente; luego podrás subirlo para enviarlo a firma.</div><a class="drive-button secondary" href="/contrato/editor/{{ registro_id }}">ABRIR EDITOR DE DOCUMENTO</a>{% else %}<p class="hint">Editor de documentos no configurado. Puedes continuar editando desde Google Drive.</p>{% endif %}</section>{% endif %}
