...
:root{font-family:Inter,system-ui,sans-serif;color:#18212f;background:#f7f9fc}.container{max-width:900px;margin:0 auto;padding:48px 20px}h1{font-size:2.2rem;margin:0 0 28px}h2{font-size:1.1rem;margin-top:0}.card,details{background:#fff;border:1px solid #dce3ed;border-radius:12px;padding:22px;margin:20px 0}summary{font-weight:700;cursor:pointer}.grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px}label{display:grid;gap:7px;font-weight:600;margin:14px 0}input,select,textarea{border:1px solid #cbd5e1;border-radius:8px;padding:10px;font:inherit;background:#fff}textarea{resize:vertical;min-height:160px}button,.drive-button{width:100%;box-sizing:border-box;display:block;text-align:center;text-decoration:none;border:0;border-radius:8px;background:#2563eb;color:#fff;font-weight:700;padding:13px;cursor:pointer;margin-top:10px}.secondary{background:#fff;color:#1d4ed8;border:1px solid #bfdbfe}.hint{color:#64748b;font-size:.9rem}.notice{padding:12px 14px;border-radius:8px;margin:14px 0}.success{background:#dcfce7;color:#166534}.error{background:#fee2e2;color:#991b1b}.info{background:#dbeafe;color:#1e40af}.modal-backdrop{position:fixed;inset:0;z-index:10;display:grid;place-items:center;padding:20px;background:rgb(15 23 42 / .55)}.modal{width:min(100%,430px);background:#fff;border-radius:18px;padding:32px;text-align:center;box-shadow:0 24px 60px rgb(15 23 42 / .3)}.modal-icon{width:58px;height:58px;margin:0 auto 18px;border-radius:50%;display:grid;place-items:center;background:#dcfce7;color:#15803d;font-size:2rem;font-weight:800}.modal h2{font-size:1.4rem;margin:0 0 10px}.modal p{margin:0;color:#475569;line-height:1.5}.modal button{margin:24px 0 0}@media(max-width:640px){.grid{grid-template-columns:1fr}.container{padding:28px 14px}}
...
</main>{% if mostrar_modal_exito %}<div class="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="modal-exito-titulo"><section class="modal"><div class="modal-icon" aria-hidden="true">✓</div><h2 id="modal-exito-titulo">{{ titulo_modal_exito }}</h2><p>{{ mensaje }}</p><button type="button" onclick="this.closest('.modal-backdrop').remove()">CONTINUAR</button></section></div>{% endif %}</body></html>""")
...
async def abrir_editor(registro_id: str):
    ...
    pagina = f'''<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Editar contrato</title><style>html,body{{height:100%;margin:0}}body{{font-family:Inter,system-ui,sans-serif;background:#f7f9fc;color:#18212f}}header{{background:#fff;border-bottom:1px solid #dce3ed}}.editor-header{{max-width:1180px;margin:0 auto;padding:18px 28px 20px}}.header-top{{display:flex;align-items:center;justify-content:space-between;gap:16px}}.back-link{{color:#1d4ed8;text-decoration:none;font-weight:700;padding:9px 12px;border-radius:8px}}.back-link:hover{{background:#eff6ff}}.status{{display:inline-flex;align-items:center;gap:7px;background:#ecfdf5;color:#047857;border:1px solid #a7f3d0;border-radius:999px;padding:7px 11px;font-size:.84rem;font-weight:700}}.status::before{{content:"";width:7px;height:7px;border-radius:50%;background:#10b981}}.editor-guide{{display:flex;align-items:end;justify-content:space-between;gap:24px;margin-top:16px}}h1{{font-size:1.45rem;margin:0 0 6px;letter-spacing:-.02em}}.editor-guide p{{margin:0;color:#64748b;line-height:1.5}}.steps{{display:flex;gap:10px;margin:14px 0 0;padding:0;list-style:none;color:#475569;font-size:.9rem}}.steps li{{display:flex;align-items:center;gap:7px}}.step-number{{display:grid;place-items:center;width:20px;height:20px;border-radius:50%;background:#dbeafe;color:#1d4ed8;font-size:.75rem;font-weight:800}}button{{border:0;border-radius:9px;background:#2563eb;color:#fff;font:inherit;font-weight:800;cursor:pointer;padding:12px 16px;white-space:nowrap;box-shadow:0 2px 5px rgb(37 99 235 / .2)}}button:hover{{background:#1d4ed8}}#editor{{height:calc(100vh - 178px);min-height:650px}}@media(max-width:700px){{.editor-header{{padding:16px}}.editor-guide{{align-items:stretch;flex-direction:column}}.steps{{flex-direction:column;gap:6px}}button{{width:100%}}#editor{{height:calc(100vh - 278px);min-height:500px}}}}</style></head><body><header><div class="editor-header"><div class="header-top"><a id="back-to-contract" class="back-link" href="/?registro={registro_id}">← Volver al contrato</a><span class="status">Borrador en edición</span></div><div class="editor-guide"><div><h1>Revisa y edita el documento</h1><p>Cuando termines, guarda los cambios en el editor y presiona <strong>Volver al contrato</strong>. El PDF se descargará automáticamente.</p><ol class="steps"><li><span class="step-number">1</span> Edita el contenido</li><li><span class="step-number">2</span> Guarda los cambios</li><li><span class="step-number">3</span> Vuelve al contrato</li></ol></div><button id="download-pdf" type="button">DESCARGAR PDF</button></div></div></header><div id="editor">Cargando editor...</div><script src="{url_api_javascript(ONLYOFFICE_DOCUMENT_SERVER_URL)}"></script><script>const config = {config_json}; config.events = {{ onError: function(event) {{ console.error("ONLYOFFICE error", event); document.getElementById("editor").textContent = "No fue posible mostrar el documento en ONLYOFFICE (código " + (event.data && event.data.errorCode || "desconocido") + "). Puedes continuar desde Google Drive."; }} }}; if (window.DocsAPI) {{ new DocsAPI.DocEditor("editor", config); }} else {{ document.getElementById("editor").textContent = "No fue posible cargar el editor. Puedes continuar desde Google Drive."; }}</script></body></html>'''
